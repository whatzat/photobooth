...
class IdleMessage(QtWidgets.QFrame):
    ...
    def __init__(self, trigger_action):
        ...
        #combine message box "label" and button to single button
        self._message_label = _('Hit the')
        self._message_button = _('Click here to start!')
...
class GreeterMessage(QtWidgets.QFrame):
    ...
    def __init__(self, num_x, num_y, skip, countdown_action):
        ...
        self._text_button = _('Starting countdown...')

        num_pictures = max(num_x * num_y - len(skip), 1)
        if num_pictures > 1:
            self._text_label = _('Get ready for {} pictures...').format(num_pictures)
        else:
            self._text_label = ''
...
class PostprocessMessage(Widgets.TransparentOverlay):
    ...
    def initFrame(self, tasks, idle_handle, worker):
        ...
        buttons.append(QtWidgets.QPushButton(_('Press button to return home')))
        ...
        # removed "Happy" message and return home button
